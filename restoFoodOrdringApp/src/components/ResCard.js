...
const ResCard = (props) => {
  ...
  return (
    <div className="bg-white shadow-lg rounded-md overflow-hidden">
      <img className="w-full h-48 object-cover" src={CDN_URL + cloudinaryImageId} alt={name} />
      <div className="p-4">
        ...
        <div className="flex items-center mb-2">
          ...
          <svg className="w-4 h-4 fill-current text-yellow-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        ...
        <div className="flex justify-between items-center">
          <span className="text-sm font-semibold text-gray-700">Cost for two:</span>
          <span className="text-sm font-semibold text-gray-700">${costForTwo}</span>
        </div>
        <div className="flex justify-between items-center mt-2">
          <span className="text-sm font-semibold text-gray-700">Delivery Time:</span>
          <span className="text-sm font-semibold text-gray-700">{resData?.info?.sla?.deliveryTime} minutes</span>
        </div>
      </div>
    </div>
  );
